!function ($) {
      ...
      $.each(columns, function (j, column) {
        ...
        if (column.selectDataValue != null && typeof column.selectDataValue === 'string') {
          if (column.selectDataType === 'var') {
            ...
            column.selectData = Object.keys(dataObject).map(function (x) { return dataObject[x]; })
          } else {
  ...
  MdlTable.prototype.f_InitHeader = function () {
    var that = this, visibleColumns = {}, html = [], htmlTotals = [];
    this.header = {
      fields: [],
      styles: [],
      classes: [],
    ...
    $.each(this.options.columns, function (i, columns) {
      ...
      if (that.f_GetChecked()) {
        if (!that.options.singleSelect) {
          html.push(Cl_Core.f_Sprintf('<th class="cbx"><label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="%s_chbxCheckAll">' +
            '<input name="bSelectAll" type="checkbox" id="%s_chbxCheckAll" class="mdl-checkbox__input mdl-checkbox__input-all"><span class="mdl-checkbox__label"></span></label></th>', that.m_ID, that.m_ID));
        } else {
      ...
      $.each(columns, function (j, column) {
        ...
        if (column.type !== "button" && column.type !== "hidden") {
          ...
          if (!that.f_GetVisible(column))
            style += 'display: none;';
          var htmlTh = ['<th' + Cl_Core.f_Sprintf(' title="%s"', column.titleTooltip),
            class_,
          Cl_Core.f_Sprintf(' style="%s"', halign + style),
          Cl_Core.f_Sprintf(' rowspan="%s"', column.rowspan),
          Cl_Core.f_Sprintf(' colspan="%s"', column.colspan),
          Cl_Core.f_Sprintf(' data-field="%s"', column.field),
            " tabindex='0'",
            '>'].join('');
          html.push(htmlTh);
          if (that.f_GetTotals()) {
    ...
    this.$container.off('click', '[name="bSelectAll"]')
      .on('click', '[name="bSelectAll"]', function () {
  ...
  MdlTable.prototype.f_OnError = function (a_Error) {
    ...
    if (this.options.errorEnabled) {
      if (this.options.errorAction !== undefined) {
        ...
        if (a_Error.msg !== undefined) { alert(a_Error.msg); }
        else if (a_Error.errmsg !== undefined) { alert(a_Error.errmsg); }
        else alert(a_Error);
      }
    }
  };
  ...
  MdlTable.prototype.f_InitToolbar = function () {
    ...
    if (this.f_GetShowingColumns()) {
      html.push(Cl_Core.f_Sprintf('<div title="%s" class="columns">', this.options.formatColumns()),
        Cl_Core.f_Sprintf('<button id="%s_bcolumns" type="button" class="mdl-button mdl-js-button mdl-button--icon" title="%s">', this.m_ID, this.options.formatColumns()),
        Cl_Core.f_Sprintf('<i class="material-icons">%s</i>', that.options.icons.columns),
        '</button>',
        Cl_Core.f_Sprintf('<ul class="mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect" for="%s_bcolumns">', this.m_ID));
      ...
      $.each(this.columns, function (i, column) {
        if (that.f_GetSwitchable(column) && column.type !== "button") {
          ...
          if (column.title.indexOf(",") > 0)
            textCol = textCol.substring(0, textCol.indexOf(","));
          html.push(Cl_Core.f_Sprintf('<li class="mdl-menu__item">' +
            '<label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="%s_chbx%s"><input type="checkbox" id="%s_chbx%s" class="mdl-checkbox__input" data-field="%s" value="%s"%s/><span class="mdl-checkbox__label">%s</span></label>' +
            '</li>', that.m_ID, i, that.m_ID, i, column.field, i, checked, textCol));
          switchableCount++;
        }
      });
      html.push('</ul>', '</div>');
    ...
    if (this.f_GetShowingColumns()) {
      ...
      var f_OnChangeListCollumns = function () {
        setTimeout(function () {
          var input = this;
      ...
      var f_OnClickMICollumns = function () {
  ...
  MdlTable.prototype.f_InitButtonsRows = function (a_Content$, a_IndexRow, a_RowData) {
    ...
    content$.find('[name="bAction"]').off('click').on('click', function () {
      ...
      if (rowData == null)
        rowData = that.f_GetRowDataFromIndex(index);
  ...
  MdlTable.prototype.f_InitChecked = function (a_Content$) {
    if (this.f_GetChecked()) {
      ...
      if (a_Content$ == null)
        that.ctrl_SelectItems = ctrl_SelectItems;
      else
        that.ctrl_SelectItems.push(ctrl_SelectItems);
  ...
  MdlTable.prototype.f_InitSelectColumns = function (a_Row$) {
    ...
    if (a_Row$ == null)
      cols$ = this.$body.find('> tr[data-index] > td' + textPnlCard);
    else
      cols$ = a_Row$.find('> td' + textPnlCard);
    cols$.off('click dblclick').on('click dblclick', function (e) {
      ...
      if (that.m_CardView && that.options.cardReviewAuto && $(e.target).parents('[name="bAction"]').length === 0)
        that.f_Open(e);
    ...
    els$.off('click').on('click', function () {
      ...
      if ($tr.next().is('tr.detail-view')) {
        ...
        if (that.options.detailShowOk && that.options.detailShowCancel) {
          textBtns = ['<div class="btns">',
            Cl_Core.f_Sprintf('<div class="btn"><button class="mdl-button mdl-js-button mdl-button--primary" type="button" name="bEdit">%s</button></div>', that.options.formatEdit()),
            '</div>'].join('');
        }
    ...
    if (this.options.footerFormatter != null) {
      ...
      if (firstRow$.length > 0) {
        $('td', firstRow$[0]).each(function () {
          html.push('<td style="',
            isFixed ? 'width:' + this.offsetWidth + 'px;' : '',
            this.hidden || this.offsetWidth === 0 ? 'display: none;' : '',
            '">&nbsp;</td > ');
        });
      }
  ...
  MdlTable.prototype.f_ResetView = function (params) {
    var padding = 0;
    if (this.f_GetChecked() && !this.options.singleSelect) {
      if (this.$selectItem.length > 0 && this.$selectItem.length === this.$selectItem.filter(':checked').length)
        this.ctrl_SelectAll.check();
      else
        this.ctrl_SelectAll.uncheck();
    }
  ...
  MdlTable.prototype.f_GetData = function (useCurrentPage) {
    return (!$.isEmptyObject(this.filterColumns) || !$.isEmptyObject(this.filterColumnsPartial)) ?
      ...
      if (column.dialogVisibleAlways || (validEditVisible && column.dialogVisible && (!column.readOnly || a_ReadOnly) && (!column.addOnly || a_RowIndex == null) && (!column.editOnly || !a_ReadOnly) && column.type !== "button" && (!column.hidden || a_ReadOnly))) {
        if (column.editDialogContent !== undefined) {
          ...
          if (column.valRegexPattern != null) {
            ...
            if (column.valRegexMsg != null)
              valid += ' patternmsg="' + column.valRegexMsg + '"';
          ...
          if (column.min != null || column.max != null) {
            if (column.min != null)
              valid += ' minlength="' + column.min + '"';
            if (column.max != null)
              valid += ' maxlength="' + column.max + '"';
          }
          var mask = "";
          ...
          if (column.type === "hidden") {
            ...
            dlgtext += [Cl_Core.f_Sprintf('<div class="field cbx%s">', column.cardColspan != null ? ' width' + column.cardColspan : column.cardWidthfull ? ' full' : ''),
            Cl_Core.f_Sprintf('<label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="%s">', id),
            Cl_Core.f_Sprintf('<input%s type="checkbox" id="%s" class="mdl-checkbox__input"%s%s />', fldAttr, id, value ? ' checked="checked"' : '', readOnlyText),
            Cl_Core.f_Sprintf('<span class="mdl-checkbox__label">%s</span>', Cl_Core.f_GetPropertyFromOther(that.columns, 'field', 'title', column.field)),
              '</label></div>'].join('');
          } else if ((column.type === "date" || column.type === "datetime") && !column.readOnly && !a_ReadOnly) {
            dlgtext += [Cl_Core.f_Sprintf('<div class="field date%s">', column.cardColspan != null ? ' width' + column.cardColspan : column.cardWidthfull ? ' full' : ''),
            Cl_Core.f_Sprintf('<div class="mdl-datefield mdl-js-datefield mdl-datefield--floating-label"><input%s data-time="%s" id="%s" class="%s mdl-datefield__input" value="%s"%s></input>',
              fldAttr, column.type === "datetime" ? "true" : "false", id, column.field, value, valid),
            Cl_Core.f_Sprintf('<label class="mdl-datefield__label" for="%s">%s</label>', id, Cl_Core.f_GetPropertyFromOther(that.columns, 'field', 'title', column.field)),
              '</div></div>'].join('');
            ...
            if (column.selectData !== undefined) {
              ...
              dlgtext += [Cl_Core.f_Sprintf('<div class="field date%s">', column.colspan != null ? ' width' + column.colspan : column.cardWidthfull ? ' full' : ''),
              Cl_Core.f_Sprintf('<div class="mdl-selectfield mdl-js-selectfield mdl-selectfield--floating-label"%s%s>', readOnlyText, column.selectReadOnly ? ' data-read-only="true"' : ''),
              Cl_Core.f_Sprintf('<input%s%s id="%s" type="text" class="mdl-selectfield__input"%s%s />', fldAttr, fldKeyAttr, id, row !== undefined ? ' key="' + Cl_Core.f_GetItemField(row, column.selectField) + '"' : '', valid),
              Cl_Core.f_Sprintf('<label class="mdl-selectfield__label" for="%s">%s</label>', id, Cl_Core.f_GetPropertyFromOther(that.columns, 'field', 'title', column.field)),
              Cl_Core.f_Sprintf('<ul for="%s">', id),
                itemsText,
                '</ul>',
                '</div></div>'].join('');
            }
            ...
            dlgtext += [Cl_Core.f_Sprintf('<div class="field%s">', column.cardColspan != null ? ' width' + column.cardColspan : column.cardWidthfull ? ' full' : ''),
            '<div class="' + a_ActionName + ' mdl-textfield mdl-textfield--floating-label is-dirty is-upgraded">',
            Cl_Core.f_Sprintf('<div class="mdl-textfield__input value">%s</div>', textVal),
            Cl_Core.f_Sprintf('<label class="mdl-textfield__label" for="%s">%s</label>', id, Cl_Core.f_GetPropertyFromOther(that.columns, 'field', 'title', column.field)),
              '</div></div>'].join('');
          } else {
            var txtType = 'text', txtMin = '', txtMax = '', txtStep = '';
            ...
            dlgtext += [Cl_Core.f_Sprintf('<div class="field%s">', column.cardColspan != null ? ' width' + column.cardColspan : column.cardWidthfull ? ' full' : ''),
            '<div class="' + a_ActionName + ' mdl-textfield mdl-js-textfield mdl-textfield--floating-label">',
            column.cardArea == null ? Cl_Core.f_Sprintf('<input%s class="mdl-textfield__input value" type="%s"%s%s%s id="%s" value="%s"%s%s%s />',
              fldAttr, txtType, txtMin, txtMax, txtStep, id, value, valid, mask, readOnlyText) :
              Cl_Core.f_Sprintf('<textarea%s class="mdl-textfield__input value" type="text" id="%s" rows="%s"%s>%s</textarea>', fldAttr, id, column.cardArea, readOnlyText, value),
            Cl_Core.f_Sprintf('<label class="mdl-textfield__label" for="%s">%s</label>', id, Cl_Core.f_GetPropertyFromOther(that.columns, 'field', 'title', column.field)),
            column.valRegexMsg != null ? Cl_Core.f_Sprintf('<span class="mdl-textfield__error">%s</span>', column.valRegexMsg) : "",
              '</div></div>'].join('');
          }
        }
      }
  ...
  MdlTable.prototype.f_Remove = function (a_Params) {
    ...
    if (this.options.removeAction != null) {
      ...
      var msg = "";
      if (this.options.removeField != null)
        msg = this.options.formatConfirmRemoveWithField() + row[this.options.removeField];
      else
        msg = this.options.formatConfirmRemove();
      if (confirm(msg)) {
        var prms = new Object();
        prms.cmd = 4;
  ...
  MdlTable.prototype.f_ActionSelected = function (a_Params) {
    ...
    if (this.options.selectConfirm != null && this.options.selectConfirm !== "") {
      if (!confirm(this.options.selectConfirm))
        return;
    }
  ...
  MdlTable.prototype.f_Detail = function (a_Params) {
    if (this.options.detailFormatter != null) {
      ...
      if (that.options.detailShowCancel)
        bCancel = {
