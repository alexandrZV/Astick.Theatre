...
!(function ($) {
  ...
  // =========================
  var MdlTable = function MdlTable(a_El, a_Options) {
    var that = this;
  ...
  MdlTable.DEFAULTS = {
    ...
    detailOkFunction: function detailOkFunction() {},
    clickToSelect: false,
    singleSelect: false,
    checkboxHeader: true,
    sortable: true,
    ...
    rowStyle: function rowStyle(a_Row, a_Index) {
      return {};
    },

    rowAttributes: function rowAttributes(a_Row, a_Index) {
      return {};
    },
    onAll: function onAll(a_Name, a_Args) {
      return false;
    },
    onClickCell: function onClickCell(a_Field, a_Value, a_Row, $element) {
      return false;
    },
    onDblClickCell: function onDblClickCell(a_Table, a_Field, a_Value, a_Index, a_Row, $element) {
      a_Table.f_CopyToBuffer($element.text());
      return false;
    },
    onClickRow: function onClickRow(a_Item, $element) {
      return false;
    },
    onDblClickRow: function onDblClickRow(a_Index, a_Row, $element) {
      return false;
    },
    f_OnSort: function f_OnSort(a_Name, a_Order) {
      return false;
    },
    f_OnCheck: function f_OnCheck(a_Row) {
      return false;
    },
    f_OnUncheck: function f_OnUncheck(a_Row) {
      return false;
    },
    f_OnCheckChange: function f_OnCheckChange(a_Row) {
      return false;
    },
    onCheckAll: function onCheckAll(a_Rows) {
      return false;
    },
    onUncheckAll: function onUncheckAll(a_Rows) {
      return false;
    },
    onCheckSome: function onCheckSome(a_Rows) {
      return false;
    },
    onUncheckSome: function onUncheckSome(a_Rows) {
      return false;
    },
    onLoadSuccess: function onLoadSuccess(a_Data) {
      return false;
    },
    onLoadError: function onLoadError(a_Status) {
      return false;
    },
    onColumnSwitch: function onColumnSwitch(a_Field, a_Checked) {
      return false;
    },
    onPageChange: function onPageChange(a_Number, a_Size) {
      return false;
    },
    f_OnToggle: function f_OnToggle(a_CardView) {
      return false;
    },
    onPreBody: function onPreBody(a_Data) {
      return false;
    },
    ...
    onPreUpdateRow: function onPreUpdateRow() {
      return false;
    },
    onPreDeleteRow: function onPreDeleteRow() {
      return false;
    },
    onExpandRow: function onExpandRow(a_Index, a_Row, $detail) {
      return false;
    },
    onCollapseRow: function onCollapseRow(a_Index, a_Row) {
      return false;
    },
    onRefreshOptions: function onRefreshOptions(a_Options) {
      return false;
    },
    onResetView: function onResetView() {
      return false;
    },
    onResetItemCards: function onResetItemCards() {
      return false;
    },
    onResetCards: function onResetCards() {
      return false;
    },
    onInitDetailView: function onInitDetailView() {
      return false;
    },
    f_OnInitDialogEdit: function f_OnInitDialogEdit() {},
    f_OnError: function f_OnError() {
      return false;
    }
  };
  ...
  MdlTable.prototype.f_GetFormatMoney = function (a_Value) {
    if (a_Value == null || a_Value === undefined || a_Value === "") return "0,00";
  ...
  MdlTable.prototype.f_GetFormatDate = function (a_Value) {
    if (a_Value != null && a_Value !== "" && a_Value !== "-" && a_Value !== "_") {
      ...
      if (date.isValid()) return date.format("DD.MM.YYYY");else return a_Value;
    } else return "";
  };
  MdlTable.prototype.f_GetFormatDateTime = function (a_Value) {
    if (a_Value != null && a_Value !== "" && a_Value !== "-" && a_Value !== "_") {
      var date = moment(a_Value);
      if (date.isValid()) return date.format("DD.MM.YYYY HH:mm:ss");else return a_Value;
    } else return "";
  };
  MdlTable.prototype.f_GetFormatMobile = function (a_Value) {
    if (a_Value.length === 10) return "(" + a_Value.substr(0, 3) + ") " + a_Value.substr(3, 3) + " " + a_Value.substr(6, 2) + " " + a_Value.substr(8, 2);else return "";
  };
  ...
  MdlTable.prototype.f_GetRowFromKey = function (a_Key) {
    if (this.options.key != null) {
      for (var i = 0; i < this.data.length; i++) {
        if (this.data[i][this.options.key].toString() === a_Key.toString()) return this.data[i];
      }
    }
    return null;
  };
  MdlTable.prototype.f_GetRowIndexFromKey = function (a_Key) {
    if (this.options.key != null) {
      for (var i = 0; i < this.data.length; i++) {
        if (this.data[i][this.options.key].toString() === a_Key.toString()) return i;
      }
    }
    return null;
  };
  ...
  MdlTable.prototype.f_GetRowDataIndexSubFromIndex = function (a_Index) {
    if (a_Index != null) {
      ...
      if (pos !== -1) return a_Index.substr(pos + 1, a_Index.length - pos - 1);else return -1;
    } else {
      return -1;
    }
  };
  ...
  MdlTable.prototype.f_GetFormatMask = function (a_Column) {
    ...
    if (a_Column.mask != null) return a_Column.mask;else if (a_Column.type === "date") return this.options.formatMaskDate();else if (a_Column.type === "datetime") return this.options.formatMaskDateTime();else if (a_Column.type === "mobile") return this.options.formatMaskMobile();else return "";
  };
  MdlTable.prototype.f_GetTextFromTemplatyText = function (a_Index, a_DataRow, a_Text, a_IsSpan) {
    ...
    if (tmpVals != null) {
      for (var i = 0; i < tmpVals.length; i++) {
        ...
        if (isSpan) text = text.replace(regex, Cl_Core.f_Sprintf('<span fld="%s">%s</span>', fld, this.f_GetValue(a_Index, fld, true, false, null, a_DataRow)));else text = text.replace(regex, this.f_GetValue(a_Index, fld, true, true, null, a_DataRow));
      }
    }
    return text;
  };
  ...
  MdlTable.prototype.f_IsVisibleButton = function (a_DataRow, a_Column) {
    return (a_Column.buttonVisibleField === undefined || !a_Column.buttonVisibleInvert && a_DataRow[a_Column.buttonVisibleField] || a_Column.buttonVisibleInvert && !a_DataRow[a_Column.buttonVisibleField]) && (a_Column.buttonVisibleAction === undefined || Cl_Core.f_CalculateObjectValue(null, a_Column.buttonVisibleAction, [this, a_DataRow], ''));
  };
  ...
  MdlTable.prototype.f_GetBtnRefresh = function () {
    if (this.options.showRefresh) return ['<div class="btn btnRefresh">', '<button title="' + this.options.formatRefresh() + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bRefresh"><span class="material-icons">' + this.options.icons.refresh + '</span></button>', this.m_CardView ? '</div>' : '</div>'].join('');else return '';
  };
  MdlTable.prototype.f_GetBtnReview = function (a_RowIndex) {
    if (this.options.showReview) return ['<div class="btn btnReview" data-key="' + this.data[a_RowIndex][this.options.key] + '">', '<button title="' + this.options.formatReview() + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bReview"><i class="material-icons">' + this.options.icons.open + '</i></button>', this.m_CardView ? '</div>' : '</div>'].join('');else return '';
  };
  MdlTable.prototype.f_GetBtnAdd = function () {
    if (this.options.showAdd) return ['<div class="btn btnAdd">', '<button title="' + this.options.formatAdd() + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bAdd"><i class="material-icons">' + this.options.icons.add + '</i></button>', this.m_CardView ? '</div>' : '</div>'].join('');else return '';
  };
  MdlTable.prototype.f_GetBtnRemoveSelected = function () {
    if (this.options.showRemoveSelected) return ['<div class="btn btnRemoveSelected">', '<button title="' + this.options.formatRemoveSelected() + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bRemoveSelected"><i class="material-icons">' + this.options.icons.remove + '</i></button>', this.m_CardView ? '</div>' : '</div>'].join('');else return '';
  };
  MdlTable.prototype.f_GetBtnActionSelected = function () {
    if (this.options.showActionSelected) return Cl_Core.f_Sprintf('<div class="btn btnActionSelected" title="%s">', this.options.selectTitle != null ? this.options.selectTitle : this.options.formatActionSelected()) + '<button class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bActionSelected">' + Cl_Core.f_Sprintf('<i class="material-icons">%s</i>', this.options.selectIcon != null ? this.options.selectIcon : this.options.icons.action) + '</button></div>';else return '';
  };
  MdlTable.prototype.f_GetBtnClone = function (a_RowIndex, a_RowData) {
    if (this.options.showClone) {
      ...
      if (this.f_IsVisibleClone(row)) {
        return ['<div class="btn btnClone" data-key="' + row[this.options.key] + '">', '<button title="' + this.options.formatClone() + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bClone"><i class="material-icons">' + this.options.icons.clone + '</i></button>', this.m_CardView ? '</div>' : '</div>'].join('');
  ...
  MdlTable.prototype.f_GetBtnEdit = function (a_RowIndex, a_RowData) {
    if (this.options.showEdit) {
      ...
      if (this.f_IsVisibleEdit(row)) {
        return ['<div class="btn btnEdit" data-key="' + row[this.options.key] + '">', '<button title="' + this.options.formatEdit() + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bEdit"><i class="material-icons">' + this.options.icons.edit + '</i></button>', this.m_CardView ? '</div>' : '</div>'].join('');
  ...
  MdlTable.prototype.f_GetBtnRemove = function (a_RowIndex, a_RowData) {
    if (this.options.showRemove) {
      ...
      if (this.f_IsVisibleRemove(row)) {
        return ['<div class="btn btnRemove" data-key="' + row[this.options.key] + '">', '<button title="' + this.options.formatRemove() + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bRemove"><i class="material-icons">' + this.options.icons.remove + '</i></button>', this.m_CardView ? '</div>' : '</div>'].join('');
    ...
    $.each(this.columns, function (i, column) {
      if (column.type === "button") {
        ...
        if (that.f_IsVisibleButton(dataRow, column)) {
          btns += Cl_Core.f_Sprintf('<div class="btn btnUsers %s" data-key="%s"><button title="%s" class="mdl-button mdl-js-button mdl-button--icon" name="bAction"%s><i class="material-icons">%s</i></button></div>', column.buttonClass, dataRow[that.options.key], column.buttonTitle, column.buttonAction != null ? ' data-action="' + column.buttonAction + '"' : '', column.title);
        }
      }
    });
    return btns;
  ...
  MdlTable.prototype.f_GetBtnDetailView = function (a_RowIndex) {
    ...
    if (this.options.detailView) {
      ...
      if (this.options.detailViewTitle != null) detailViewTitle = this.options.detailViewTitle;
      if (this.options.detailViewIcon != null) detailIcon = this.options.detailViewIcon;
      row = this.data[a_RowIndex];
      if (this.f_IsVisibleDetail(row)) {
        return [this.m_CardView ? '<div class="btn" data-key="' + row[this.options.key] + '">' : '<td class="btn" data-key="' + row[this.options.key] + '">', '<button title="' + detailViewTitle + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bDetail"><i class="material-icons">' + detailIcon + '</i></button>', this.m_CardView ? '</div>' : '</td>'].join('');
      } else return '';
      ...
      if (row.p_SubData != null && row.p_SubData.length > 0) {
        ...
        if (this.options.subDetailTitle != null) subTitle = this.options.subDetailTitle;
        if (this.options.subDetailIcon != null) subIcon = this.options.subDetailIcon;
        return [this.m_CardView ? '<div class="btn" data-key="' + this.data[a_RowIndex][this.options.key] + '">' : '<td class="btn" data-key="' + this.data[a_RowIndex][this.options.key] + '">', '<button title="' + subTitle + '" class="mdl-button mdl-js-button mdl-button--icon" type="button" name="bSubDetail"><i class="material-icons">' + subIcon + '</i></button>', this.m_CardView ? '</div>' : '</td>'].join('');
      }
    } else return '';
  };
  ...
  MdlTable.prototype.f_HasHiddenColumn = function (field) {
    ...
    if (column != null) return !this.f_GetVisible(column);else return true;
  };
  MdlTable.prototype.f_HasVisibleColumn = function (field) {
    var column = this.f_GetColumn(field);
    if (column != null) return this.f_GetVisible(column);else return false;
  };
  ...
  MdlTable.prototype.f_HideLoading = function (a_Index) {
    if (this.m_CardView) {
      ...
      if (a_Index != null) {
        var elRow = this.el_Table.querySelector('tbody tr[data-index="' + a_Index + '"]');
        if (elRow.nextSibling != null) elRow.nextSibling.remove();
      } else {
  ...
  MdlTable.prototype.f_GetValueByTypeRow = function (a_Type, a_Value, a_IsUnit, a_IsDialog) {
    ...
    if (a_Type === "money") {
      ...
      if (a_IsUnit) {
        if (this.m_CardView || isDialog) txtHtml += " " + this.options.formatMoney();else txtHtml += " <span class='moneySymbol'>" + this.options.formatMoney() + "<span>";
      }
  ...
  MdlTable.prototype.f_GetValue = function (a_Index, a_Field, a_IsUnit, a_IsDialog, a_DataRows, a_Item) {
    var item = a_Item;
    if (item == null) item = a_DataRows != null ? a_DataRows[a_Index] : this.f_GetRowDataFromIndex(a_Index);
    if (item == null) return '';
    ...
    if (column != null) {
      if (!a_IsDialog && column.selectData !== undefined && column.selectData !== null) {
        ...
        if (column.type === "array") {
          ...
          if (this.f_IsVisibleButton(item, column)) value = Cl_Core.f_Sprintf('<button name="bAction" class="mdl-button mdl-js-button mdl-button--icon %s"%s href="javascript:void(0)"%s><i class="material-icons">%s</i></button>', column.buttonClass, column.buttonTitle != null ? ' title="' + column.buttonTitle + '"' : '', column.buttonAction != null ? ' data-action="' + column.buttonAction + '"' : '', column.title);else value = '';
        } else {
          if (!a_IsDialog) {
            value = Cl_Core.f_CalculateObjectValue(column, this.header.formatters[column.fieldIndex], [value, item, a_Index], value);
          }
  ...
  MdlTable.prototype.f_GetColumnCheckedHtmlText = function (a_RowIndex, a_RowData) {
    ...
    if (this.f_GetChecked()) {
      if (this.f_IsVisibleSelect(a_RowData)) {
        var text = ['<td class="cbx mdl-table_row_cbx">', Cl_Core.f_Sprintf('<label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="%s_chbxCheckRow%s"><input', this.m_ID, a_RowIndex) + Cl_Core.f_Sprintf(' id="%s_chbxCheckRow%s" class="mdl-checkbox__input"', this.m_ID, a_RowIndex) + Cl_Core.f_Sprintf(' data-index="%s"', a_RowIndex) + Cl_Core.f_Sprintf(' name="%s"', this.options.selectItemName) + Cl_Core.f_Sprintf(' type="%s"', 'checkbox') + Cl_Core.f_Sprintf(' value="%s"', a_RowData[this.options.idField]) + ' /><span class="mdl-checkbox__label"></span></label>', this.m_CardView ? '</div>' : '</td>'].join('');
        html = text;
      } else {
  ...
  MdlTable.prototype.f_GetRowHtmlText = function (a_Index, a_Classes, a_DataRows, a_RowDataIndex) {
    ...
    if (a_Classes != null) {
      if (style.classes != null) style.classes += "mdl-table_row " + a_Classes;else style.classes = "mdl-table_row " + a_Classes;
    } else {
    ...
    html.push('<tr', Cl_Core.f_Sprintf(' %s', htmlAttributes.join(' ')), Cl_Core.f_Sprintf(' id="%s"', $.isArray(item) ? undefined : item._id), Cl_Core.f_Sprintf(' class="%s"', style.classes || ($.isArray(item) ? undefined : item._class)), Cl_Core.f_Sprintf(' data-index="%s"', attrIndex), Cl_Core.f_Sprintf(' data-uniqueid="%s"', item[this.options.uniqueId]), Cl_Core.f_Sprintf('%s', data_), '>');
    ...
    if (this.m_CardView && that.options.cardTemplate != null) {
      ...
      if (!this.m_CardView) {
        ...
        if (this.options.detailView || this.options.detailSub) {
          if (this.options.detailView && this.f_IsVisibleDetail(item) || this.options.detailSub && item.p_SubData != null && item.p_SubData.length > 0) {
            html.push('<td class="detail">', '<a class="detail-icon" href="javascript:">', Cl_Core.f_Sprintf('<i class="material-icons %s"></i>', this.options.icons.detailOpen), '</a>', '</td>');
          } else {
      ...
      $.each(this.header.fields, function (j, field) {
        ...
        if (column.type !== 'hidden') {
          ...
          style = Cl_Core.f_Sprintf('style="%s%s%s"', csses.concat(that.header.styles[j]).join('; '), !that.m_CardView && column.wordWrap ? 'word-wrap: break-word;min-width: ' + column.wordWrapMinWidth + 'px;max-width: ' + column.wordWrapMaxWidth + 'px;white-space:normal;' : '', that.f_GetVisible(column) ? '' : ' display: none');
          ...
          if (that.m_CardView) {
            ...
            if (column.type === "button") {
              ...
              if (that.f_IsVisibleButton(item, column)) {
                var buttonTitle = "",
                    buttonAction;
                if (column.buttonTitle != null) buttonTitle = ' title="' + column.buttonTitle + '"';
                if (column.buttonAction != null) buttonAction = ' data-action="' + column.buttonAction + '"';
              ...
              text += txtTemp;
              text += '</td>';
            } else if (column.fieldLink !== undefined && column.fieldLink !== null && column.fieldLink !== "" || column.fieldLinkUrl !== undefined && column.fieldLinkUrl !== null && column.fieldLinkUrl !== "" || column.fieldLinkFormatter !== undefined && column.fieldLinkFormatter !== null && column.fieldLinkFormatter !== "") {
              var linkValue = '';
  ...
  // ==========================
  var allowedMethods = ['f_GetSelections', 'f_GetAllSelections', 'f_GetData', 'f_Load', 'f_Open', 'f_Review', 'f_Add', 'f_Clone', 'f_Edit', 'f_Remove', 'f_RemoveSelected', 'f_GetColumnsVisible', 'f_GetColumnsHidden', 'f_CheckAll', 'f_UncheckAll', 'f_Check', 'f_Uncheck', 'f_Refresh', 'f_ResetView', 'f_Destroy', 'f_ShowLoading', 'f_HideLoading', 'f_ShowColumn', 'f_HideColumn', 'f_ScrollTo', 'f_InitCardView'];
  ...
  $.fn.mdlTable = function (option) {
    ...
    this.each(function () {
      var $this = $(this),
          ...
          options = $.extend({}, MdlTable.DEFAULTS, $this.data(), typeof option === 'object' && option);
