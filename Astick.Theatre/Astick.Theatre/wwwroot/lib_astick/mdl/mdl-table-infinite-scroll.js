        ...
        if (el.scrollHeight - scrollTop <= el.clientHeight + offset) {
          if (that.data.length < that.options.totalRows) {
            ...
            if (that.options.pageFixedSize) {
              ...
              if ((that.options.pageNumber + 1) * that.options.pageSize > that.options.totalRows) {
                countOpenItems = that.options.totalRows - that.options.pageNumber * that.options.pageSize;
              }
              ...
              if (countOpenItems < minCountOpenItems)
                countOpenItems = minCountOpenItems;
  ...
  MdlTable.prototype.f_InitServer = function (silent, query, callback, a_Index, a_CountOpenItems) {
    ...
    if (!that.options.pageFixedSize) {
      ...
      if (that.options.pageNumber === 1)
        params.limit = this.options.pageSize * 3;
      else
        params.limit = a_CountOpenItems !== undefined ? a_CountOpenItems : this.options.pageSize;
    } else {
